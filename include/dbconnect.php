<?php
/*
 * Core file for library and parameter handling: - $LastChangedDate$ - $Rev$
 */
...
if (! isset ( $pdfprint )) {
	...
	if (ini_get ( 'zlib.output_compression' ) != 1 && isset ( $compression_level ) && $compression_level > 0) {
...
if (! isset ( $mail_as_image ))
	$mail_as_image = false;
	
	// Use images for e-mail addresses
if (! isset ( $mail_accept ))
	$mail_accept = array ();
...
$use_sso = (isset ( $sso_facebook_id ) && $sso_facebook_id != "") || (isset ( $sso_google_id ) && $sso_google_id != "") || (isset ( $sso_yahoo_id ) && $sso_yahoo_id != "") || (isset ( $sso_hotmail_id ) && $sso_hotmail_id != "") || (isset ( $sso_twitter_id ) && $sso_twitter_id != "");

// Enable "doodle" mode, if keys are availabe
$use_doodle = isset ( $doodle ['key'] ) && isset ( $doodle ['secret'] );

// Define default ZIP handling
if (isset ( $plz_pattern ))
	$zip_pattern = $plz_pattern;
if (! isset ( $zip_pattern ))
	$zip_pattern = "";
...
if (! isset ( $default_languages ))
	$default_languages = "ar,de,fr,it,th,ru";
...
if ($group_name == "") {
	...
	if ($group_name == "[none]" || $group_name == "[no group]") {
		...
		$base_where .= "AND $table.id = $table_grp_adr.id " . "AND $table_grp_adr.group_id  = $table_groups.group_id " . "AND $table_groups.group_name = '$group_name'";
	} else {
		$base_select = "DISTINCT $table.*";
		$base_from = "$table_grp_adr, $sql_from, $table";
		$base_where .= "AND $table.id = $table_grp_adr.id " . "AND $table_grp_adr.group_id  = g0.group_id " . "AND ($sql_where)";
	}
}
...
if ($position_filter == "") {
	...
	elseif($position_filter =='[FORWARDS]') {
		$base_where .= " AND ( position_1=1 OR position_2=1 OR position_3=1 OR position_4=1 OR position_5=1 OR position_6=1 OR position_7=1 OR position_8=1 OR position_9=1 ) ";
	}
	elseif($position_filter =='[BACKS]') {
		$base_where .= " AND ( position_10=1 OR position_11=1 OR position_12=1 OR position_13=1 OR position_14=1 OR position_15=1 ) ";
	}
	elseif($position_filter =='[PROP]') {
		$base_where .= " AND ( position_1=1 OR position_3=1) ";
	}
	elseif($position_filter =='[HOOKER]') {
		$base_where .= " AND ( position_2=1) ";
	}
	elseif($position_filter =='[LOCKS]') {
		$base_where .= " AND ( position_4=1 OR position_5=1) ";
	}
	elseif($position_filter =='[FLANKER]') {
		$base_where .= " AND ( position_6=1 OR position_7=1) ";
	}
	elseif($position_filter =='[NUMBER 8]') {
		$base_where .= " AND ( position_8=1) ";
	}
	elseif($position_filter =='[SCRUM_HALF]') {
		$base_where .= " AND ( position_9=1) ";
	}
	elseif($position_filter =='[FLY_HALF]') {
		$base_where .= " AND ( position_10=1) ";
	}
	elseif($position_filter =='[CENTRE]') {
		$base_where .= " AND ( position_12=1 OR position_13=1) ";
	}
	elseif($position_filter =='[WING]') {
		$base_where .= " AND ( position_11=1 OR position_14=1) ";
	}
	elseif($position_filter =='[FULL-BACK]') {
		$base_where .= " AND ( position_15=1) ";
	}
}
...
if ($role_filter == "") {
	...
	elseif($role_filter =='[PRESIDENT]') {
		$base_where .= " AND ( president=1 ) ";
	}
	elseif($role_filter =='[VICEPRESIDENT]') {
		$base_where .= " AND ( vicepresident=1 ) ";
	}
	elseif($role_filter =='[SECRETARYGENERAL]') {
		$base_where .= " AND ( secretarygeneral=1 ) ";
	}
	elseif($role_filter =='[TREASURER]') {
		$base_where .= " AND ( treasurer=1 ) ";
	}
	elseif($role_filter =='[COMMUNICATION]') {
		$base_where .= " AND ( communication=1 ) ";
	}
	elseif($role_filter =='[REFEREE]') {
		$base_where .= " AND ( referee=1 ) ";
	}
	elseif($role_filter =='[TRAINER]') {
		$base_where .= " AND ( trainer=1 ) ";
	}
